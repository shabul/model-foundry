"""
Win-rate evaluation: Gemini judges SFT responses vs DPO responses side-by-side.
Reports what % of the time DPO wins over SFT.
...
def get_response(model, tokenizer, prompt: str) -> str:
    out = generate(model, tokenizer, prompt=prompt, max_tokens=MAX_TOKENS, temp=0.3, verbose=False)
    if "[/INST]" in out:
...
def main():
    ...
    for path, name in [(SFT_ADAPTER, "SFT adapter"), (DPO_ADAPTER, "DPO adapter")]:
    ...
    dpo_model, dpo_tok = load(BASE_MODEL, adapter_path=DPO_ADAPTER)
    client = genai.Client(api_key=api_key)

    results = {"dpo_wins": 0, "sft_wins": 0, "tie": 0}
    print(f"\nEvaluating {N_EVAL} prompts — Gemini judge\n{'='*55}")
    ...
    for i, ex in enumerate(examples, 1):
        ...
        dpo_resp = get_response(dpo_model, dpo_tok, prompt)

        # Randomly assign A/B to avoid position bias
        if random.random() < 0.5:
            a, b, mapping = sft_resp, dpo_resp, {"A": "sft", "B": "dpo"}
        else:
            a, b, mapping = dpo_resp, sft_resp, {"A": "dpo", "B": "sft"}
        ...
        print(f"  [{i:3d}/{N_EVAL}] winner={actual_winner:3s} | {reason[:60]}")

    total = N_EVAL
    dpo_rate = results["dpo_wins"] / total * 100
    sft_rate = results["sft_wins"] / total * 100

    print(f"\n{'='*55}")
    print(f"Results over {total} prompts:")
    print(f"  DPO wins : {results['dpo_wins']:3d} / {total}  ({dpo_rate:.1f}%)")
    print(f"  SFT wins : {results['sft_wins']:3d} / {total}  ({sft_rate:.1f}%)")
    print(f"  Ties     : {results['tie']:3d} / {total}")
    print(f"\nDPO win rate vs SFT: {dpo_rate:.1f}%")

    out = os.path.join(os.path.dirname(__file__), "win_rate_results.json")
    with open(out, "w") as f:
        json.dump({"dpo_win_rate": dpo_rate, "sft_win_rate": sft_rate,
